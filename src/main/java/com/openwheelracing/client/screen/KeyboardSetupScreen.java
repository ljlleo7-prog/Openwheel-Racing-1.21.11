...
public class KeyboardSetupScreen extends Screen {
    ...
    protected void rebuildWidgets() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("screen.openwheelracing.keyboard_setup.cancel"), button -> closeToParent())
            .bounds(x + 288, y + 224, 76, 20).build());
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        int x = (width - PANEL_WIDTH) / 2;
        int y = 28;
        graphics.fill(x, y, x + PANEL_WIDTH, Math.min(height - 8, y + 254), 0xDD1F2328);
        graphics.fill(x + 6, y + 20, x + PANEL_WIDTH - 6, y + 152, 0xFF2A3038);
        graphics.fill(x + 6, y + 158, x + PANEL_WIDTH - 6, y + 216, 0xFF2F3640);
        graphics.drawString(font, title, x + 10, y + 6, 0xFFE8EDF2, false);
        graphics.drawString(font, Component.translatable("screen.openwheelracing.keyboard_setup.response"), x + 12, y + 160, 0xFFC9D1D9, false);
        drawPreview(graphics, x + 98, y + 176);
    ...
    private void drawPreview(GuiGraphics graphics, int x, int y) {
        float throttle = keyDown(OWRKeyMappings.THROTTLE) ? 1.0f : 0.0f;
        float brake = keyDown(OWRKeyMappings.BRAKE) ? 1.0f : 0.0f;
        drawBar(graphics, x, y, 170, throttle, 0.0f, 1.0f, 0xFF7EE787, Component.translatable("screen.openwheelracing.keyboard_setup.preview_throttle"), String.format(Locale.ROOT, "%.0f%%", throttle * 100.0f));
        drawBar(graphics, x, y + 12, 170, brake, 0.0f, 1.0f, 0xFFFF7B72, Component.translatable("screen.openwheelracing.keyboard_setup.preview_brake"), String.format(Locale.ROOT, "%.0f%%", brake * 100.0f));
