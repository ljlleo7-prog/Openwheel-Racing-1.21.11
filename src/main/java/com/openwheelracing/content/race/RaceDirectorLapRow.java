...
public record RaceDirectorLapRow(long id, String driverName, int lapTicks, int checkpointCount, boolean invalidated, String invalidationReason, long startFinishPos, int power, int grip, int aero, int gearing, int damagePercent, int tyreWearPercent, boolean absEnabled, long sessionId, String sessionName) {
    public static RaceDirectorLapRow fromRecord(OWRLapRecords.LapRecord record) {
        OWRLapRecords.CarSnapshot car = record.car();
        return new RaceDirectorLapRow(record.id(), record.driverName(), record.lapTicks(), record.checkpointCount(), record.invalidated(), record.invalidationReason(), record.startFinishPos(), car.power(), car.grip(), car.aero(), car.gearing(), car.damagePercent(), car.tyreWearPercent(), car.absEnabled(), record.sessionId(), record.sessionName());
    }
